...
| **`Invoice.IsPotentialDuplicate` / `Invoice.DuplicateCheckReason`** — new persisted fields on `Invoice` (bool / nullable string). `DuplicateDetectionService` stays a pure compute service (no `SaveChangesAsync`); the ingestion orchestrator (`InvoiceProcessingService`, WP-012) calls `CheckAsync` and persists the result via `IInvoiceRepository`, wired in automatically after extraction/before an invoice is first surfaced to a reviewer. | Next touch of WP-010 / the ingestion pipeline | **Medium** — closes a real gap before approval workflow lands | `docs/WP-010-Duplicate-Flag-Persistence-Decision.md` |
