...
| WP-057 | Retire `DUPLICATE_SUSPECTED` Status Row | Backend Engineer | Done. Completes the `StatusReference` deletion WP-053 deferred (its own scope was transition enforcement, not status catalogue changes) — both templates' `DUPLICATE_SUSPECTED` rows removed via migration `RemoveDuplicateSuspectedStatus`; `InvoiceStatusCodes.DuplicateSuspected` (the C# constant) deliberately kept for any historical `Invoice.Status` value. This WP's own source drop predated WP-056; only the files WP-057 itself owns were merged, and its EF migration was regenerated fresh against `main`. **Consequence, not yet acted on:** `docs/AI/06_Domain_Reference_Data.md` §2 still lists `DUPLICATE_SUSPECTED` in the platform-default catalogue — that canonical document is the Chief Technical Architect's/Product Owner's to edit, not this WP's, so it remains stale until updated at the source (see the WP-053 note below) — see `docs/WP-057-Retire-Duplicate-Suspected-Status-Decisions.md` |
...
- `docs/WP-053-Transition-Enforcement-Decisions.md` — both discrepancies resolved; the `DUPLICATE_SUSPECTED` `StatusReference` row cleanup is now **closed by WP-057** (see that row above and `docs/WP-057-Retire-Duplicate-Suspected-Status-Decisions.md`). **Note:** this ruling asserts `docs/AI/06_Domain_Reference_Data.md` already has `DUPLICATE_SUSPECTED` removed from its catalogue — as of this update, the tracked copy in this repository still lists it; that file has not yet been edited to match (flagged back to the Chief Technical Architect, not silently corrected — this remains open even after WP-057, since editing the canonical reference document isn't a Backend Engineer work package's call to make).
