...
- `docs/WP-018-Invoice-Workflow-Actions-Decisions.md` (the transition-graph gap it flagged is resolved by WP-053, and the backend endpoint it flagged as missing is now delivered by WP-054 — `GET .../available-actions` / `PATCH .../status`; the frontend fixture data itself still hardcodes zero actions for platform-default tenants and needs to be replaced with the real HTTP client, tracked in `docs/Backlog.md`)
