...
| WP-021 | Azure Infrastructure (App Service, SQL, Storage) | DevOps Engineer | Infra code merged (Bicep + scripts, dev-scope only) and verified compiling — a Bicep string-escaping bug (`''` instead of `\'`) in `main.bicep` was fixed during the merge (mechanical, no semantic change). **Not yet deployable**: dev Entra External ID (CIAM) tenant doesn't exist yet (needs a human with Tenant Creator/Global Admin), and the `Mail.ReadWrite` Graph permission has no mailbox to target if left in that CIAM tenant (needs Chief Technical Architect ruling on which tenant hosts it) — see `docs/WP-021-Azure-Infrastructure-Decisions.md` |
