...
| WP-019 | Supplier Folder View | Senior React Engineer | Not started. **Note:** WP-020 (below) is scoped to swap WP-019's fixture client for a real one alongside WP-015/016/017/018's — but WP-019 itself hasn't been built yet, fixture or otherwise, unlike those four. Flagging the sequencing question rather than assuming; may need WP-019 delivered first. |
| WP-020 | API Integration & Global Error Handling | Senior React Engineer | Not started. **Assigned and amended 2026-07-25** (Chief Technical Architect ruling) — this is the originally-planned WP-020 arriving at the point it was always needed, not new scope: real MSAL sign-in (replacing WP-014's demo stub entirely), central API client with interceptors/error handling/session-expiry/retry, per-environment config, AND swapping all of WP-015/016/017/018/019's fixture clients for the real one in this same WP, with WP-018's role-gating reconciled against WP-053's full 4-transition set. Depends on WP-058 (below) and a real/dev-tier Entra External ID tenant (DevOps). Not yet dropped for merge. |
...
| WP-058 | Restore Invoice List Endpoint & Minor Doc Fix | Backend Engineer | Assigned 2026-07-25 (Chief Technical Architect ruling), not yet dropped for merge. Re-adds `GET /api/invoices` to `InvoicesController`, wiring the existing, already-tested `IInvoiceQueryService` (WP-011) — controller-only, no service-layer changes. Also fixes the stale comment in `InvoiceStatusCodes.cs` lines 72–75 (claims GB Skips' transitions are unconfirmed; WP-053 confirmed and enabled all of them). WP-020 depends on this landing first. |
