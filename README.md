...
| WP-022 | CI/CD Pipeline (GitHub Actions) | DevOps Engineer | Development-only pipeline delivered (`.github/workflows/ci-cd.yml`, `docs/CI-CD-Pipeline.md`, `infra/scripts/setup-github-oidc-service-principal.sh`, `infra/scripts/grant-ci-sql-migration-access.sql`). Backend build/test/publish and frontend build/lint/type-check/test/build all verified locally against this repo (325/325 backend tests, 187/187 frontend tests, clean `eslint`/`tsc -b`/`npm run build`); Azure resource names, subscription/tenant IDs, and Entra client IDs referenced in the docs independently verified against the live `rg-apflow-dev` resource group and Entra tenant. **Not yet run for real** (no GitHub Actions execution in this session) — three items the WP's own docs flag as unverified: the OIDC→`dotnet ef` federated-token hand-off, the `Website Contributor` RBAC role's sufficiency, and (confirmed as a real gap, not just caution) `deploy-web`'s publish step expects `src/APFlow.Web/server.js`, which **does not exist** in this repo — `APFlow.Web` has no static-file server yet, so a real run of `deploy-web` would ship an App Service with nothing to serve `dist/` — see `docs/Backlog.md`. |
