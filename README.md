...
| WP-022 | CI/CD Pipeline (GitHub Actions) | DevOps Engineer | Development-only pipeline delivered (`.github/workflows/ci-cd.yml`, `docs/CI-CD-Pipeline.md`, `infra/scripts/setup-github-oidc-service-principal.sh`, `infra/scripts/grant-ci-sql-migration-access.sql`). Backend build/test/publish and frontend build/lint/type-check/test/build all verified locally against this repo (325/325 backend tests, 187/187 frontend tests, clean `eslint`/`tsc -b`/`npm run build`); Azure resource names, subscription/tenant IDs, and Entra client IDs referenced in the docs independently verified against the live `rg-apflow-dev` resource group and Entra tenant. **First real GitHub Actions run (2026-07-31) confirmed the OIDC→Azure federated-token hand-off itself works** (the pipeline reached the `dotnet ef database update` step, meaning login succeeded) but failed there with `APFlow.Api doesn't reference Microsoft.EntityFrameworkCore.Design` — fixed by adding that package to `APFlow.Api.csproj` (`PrivateAssets="all"`, matching Infrastructure's existing design-time-only reference); verified locally that `dotnet ef migrations list` now works against the same `--project`/`--startup-project` flags the pipeline uses. Still unverified: the actual Azure-AD-authenticated SQL connection from that step (the run never reached it), and the `Website Contributor` RBAC role's sufficiency for `deploy-web`. `deploy-web`'s publish step still expects `src/APFlow.Web/server.js`, which **does not exist** in this repo — see `docs/Backlog.md`. |
